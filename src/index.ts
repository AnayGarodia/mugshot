...
function looksFem(seed: string, r: Rng): boolean {
  ...
  if (/(endra|indra|itya|esh|arsh|ansh|jit|jeet|deep|raj|inder|veer)$/.test(name)) return false;
  if (/(ee|ti|ini|ika|ali|ari)$/.test(name) && name.length > 4) return true;
