...
let femTable: Set<string> | null = null;
let mascTable: Set<string> | null = null;
let gramW: Map<string, number> | null = null;
...
let b64cache: number[] | null = null;
function atob_(s: string): number[] {
  if (b64cache) return b64cache;
...
function looksFem(seed: string, r: Rng): boolean {
  ...
  if (name.length < 3 || !/^[a-z]+$/.test(name)) return chance(r, 0.22);
  if (!femTable) initGender();
  if (femTable!.has(name)) return true;
  if (mascTable!.has(name)) return false;
  const s2 = "^" + name + "$";
...
function build(seed: string, options: FaceOptions = {}): FaceParts {
  ...
  if (chance(rc, colorOn ? 0.3 : 0.12)) { // blush
    for (const s of [-1, 1]) {
      ...
      const pts: Pt[] = [];
      for (let i = 0; i < 2; i++) pts.push([bx - 4.5 + rand(rc, -0.5, 0.5), chY + i * 2], [bx + 4.5 + rand(rc, -0.5, 0.5), chY + i * 2 + 0.5]);
  ...
  if (hairKind === "solid" || hairKind === "cap" || hairKind === "hatch") {
    ...
    if (fem) pts2.push([cx - edgeX(sideDrop) * 0.9, sideDrop + 3]);
