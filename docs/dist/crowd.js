...
function wob(r, pts, close = false, w = 1.1) {
    const j = pts.map(([x, y]) => [x + rand(r, -w, w), y + rand(r, -w, w)]);
    if (j.length === 2)
...
function person(seed, colorOn, paper = "#f4f1ea") {
    ...
    const P = [];
    const path = (d, o = {}) => P.push(`<path d="${d}" fill="${o.fill ?? "none"}" stroke="${ink}" stroke-width="${o.w ?? 1.6}"${o.op ? ` opacity="${o.op}"` : ""} stroke-linecap="round" stroke-linejoin="round"/>`);
    const cxp = 55;
    ...
    const shL = [cxp - half * 0.9, shY + 8], shR = [cxp + half * 0.9, shY + 8];
    const mitt = (x, y) => P.push(`<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="2.6" fill="none" stroke="${ink}" stroke-width="1.4"/>`);
    const armDown = (sh, dir) => {
...
export function crowd(seeds, options = {}) {
    ...
    for (let r = 0; r < rows; r++)
        rowsSeeds.push(seeds.slice(r * perRow, (r + 1) * perRow));
